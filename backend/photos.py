...
def photos(
    ...
    if q.strip():
        conditions.append(
            or_(
                func.instr(func.lower(Photo.filename), func.lower(q.strip())) > 0,
                func.instr(Photo.taken_at, q.strip()) > 0,
            )
        )
    if album:
        conditions.append(
...
async def upload(request: Request, filename: str = Query(..., min_length=1, max_length=255)):
    ...
    # One decoder at a time bounds peak memory on the Raspberry Pi.
